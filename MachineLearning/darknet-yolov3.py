...
#追加
print(os.getcwd())
lib = CDLL("/home/admin_107/Desktop/darknet/libdarknet.so", RTLD_GLOBAL)
...
def detect(net, meta, image, alp, thresh=.5, hier_thresh=.5, nms=.45):
    ...
    for j in range(num):
        for i in range(meta.classes):
            if dets[j].prob[i] > 0:
                b = dets[j].bbox
                if res.get(meta.names[i],0) < dets[j].prob[i]:
                    res[meta.names[i].decode()] = dets[j].prob[i]
...
def predict(file_path):
    ...
    net = load_net(CFG.encode('ascii'), WEIGHTS.encode('ascii'), 0)
    meta = load_meta("cfg/coco.data".encode('ascii'))
    alphabet = load_alphabet()
    r = detect(net, meta, FILE_PATH.encode('ascii'), alphabet)
